...
void FitCF::Output_results(int mode)
{
	...
	filename_stream_HBT_g0 << global_path << "/HBTradii_" << modeString << "ev" << currentfolderindex << no_df_stem << "_grid0.dat";
	...
	filename_stream_HBTcfs << global_path << "/HBTradii_" << modeString << "cfs_ev" << currentfolderindex << no_df_stem << ".dat";
	...
	for (int iKT = 0; iKT < n_localp_T; ++iKT)
	{
		...
		if (mode == 0)
		{
			...
			{
				outputHBTcfs << currentfolderindex << "  " << SPinterp_pT[iKT] << "  " << Morder
...
void FitCF::Readin_total_target_eiqx_dN_dypTdpTdphi(int folderindex)
{
	...
	for (int ipphi = 0; ipphi < n_interp_pphi_pts; ++ipphi)
	{
		...
		input_target_dN_dypTdpTdphi >> dummy;
		current_dN_dypTdpTdphi_moments[ipt][ipphi][iqt][iqx][iqy][iqz][0] += dummy;
		input_target_dN_dypTdpTdphi >> dummy;
		current_dN_dypTdpTdphi_moments[ipt][ipphi][iqt][iqx][iqy][iqz][1] += dummy;
		input_target_dN_dypTdpTdphi >> dummy;
